    ...
    <section id="work" className="py-20 lg:py-28 bg-slate-50 relative">
      <div className="container mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-4">
          {caseStudies.slice(0, 6).map((cs) => (
            <Link
              ...
              className="group block bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl hover:translate-y-[-4px] transition-all duration-300 mt-6"
            >
              {/* Image Container */}
                  ...
                  <>
                    <div
                      className={`absolute inset-0 bg-gradient-to-br transition-opacity duration-500 ${
                        cs.id.includes("ecommerce")
                          ? "from-blue-500/10 to-purple-500/10"
                          : cs.id.includes("health")
              ...
              <div className="p-8">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {cs.technologies.slice(0, 3).map((tech) => (
                    <span
                      key={tech}
                      className="px-2.5 py-1 rounded-md bg-slate-50 text-slate-500 text-xs font-medium border border-slate-100"
                    >
                      {tech}
                    </span>
                  ))}
        ...
        <div className="text-center mt-16">
          <Link
            href="/case-studies"
            className="inline-flex items-center justify-center gap-2 px-8 py-3 text-base font-semibold rounded-full bg-white text-slate-900 border border-slate-200 hover:border-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 transition-all duration-300"
          >
            View All Projects
          </Link>
        </div>
